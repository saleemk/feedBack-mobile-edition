...
function New-RouterFixture {
    param([switch]$UnicodePath)

    $pathMarker = if ($UnicodePath) {
        " $([char]0x00F1) "
    } else {
        ' '
    }
    $root = Join-Path -Path ([System.IO.Path]::GetTempPath()) -ChildPath ("mobile edition router test$pathMarker" + [guid]::NewGuid().ToString('N'))
...
function Get-TestCompanionDataRoot {
    param([string]$RepositoryRoot)

    Join-Path -Path $RepositoryRoot -ChildPath 'companion data'
}

function Set-CurrentInstallationRecord {
    param(
        [string]$CompanionDataRoot,
        [string]$Tag
    )

    New-Item -ItemType Directory -Path $CompanionDataRoot -Force | Out-Null
    Set-Content -LiteralPath (Join-Path -Path $CompanionDataRoot -ChildPath 'current-installation.json') -Value @"
{"schema":"feedback-mobile-edition.current-installation.v1","currentTag":"$Tag"}
"@ -Encoding UTF8
}

function Set-CurrentInstallationRecordRaw {
    param(
        [string]$CompanionDataRoot,
        [string]$Content
    )
    ...
    New-Item -ItemType Directory -Path $CompanionDataRoot -Force | Out-Null
    Set-Content -LiteralPath (Join-Path -Path $CompanionDataRoot -ChildPath 'current-installation.json') -Value $Content -Encoding UTF8
}

function Set-SetupBundleManifest {
    param(
    ...
    Set-Content -LiteralPath (Join-Path -Path $InstallationRoot -ChildPath 'SETUP-BUNDLE-MANIFEST.json') -Value @"
{
  ...
  "companionSha256": "$Sha256",
  "generatedAtUtc": "2026-09-09T00:00:00Z"
}
"@ -Encoding UTF8
}

function New-ManagedInstallationFixture {
    param(
        [string]$CompanionDataRoot,
        [string]$Tag,
        [string]$CompanionContent = 'managed setup companion bytes'
    )

    $installRoot = Join-Path -Path (Join-Path -Path $CompanionDataRoot -ChildPath 'installations') -ChildPath $Tag
    New-Item -ItemType Directory -Path (Join-Path -Path $installRoot -ChildPath 'scripts') -Force | Out-Null
    Set-TestBinaryFile -Path (Join-Path -Path $installRoot -ChildPath 'Setup-MobileEdition.exe') -Content $CompanionContent
    Set-TestBinaryFile -Path (Join-Path -Path $installRoot -ChildPath 'Setup-MobileEdition.cmd') -Content '@echo off'
    Set-TestBinaryFile -Path (Join-Path -Path $installRoot -ChildPath '.env.example') -Content 'LIBRARY_PATH=./library'
    Set-TestBinaryFile -Path (Join-Path -Path $installRoot -ChildPath 'docker-compose.release.yml') -Content 'name: feedback-mobile-edition'
    Set-TestBinaryFile -Path (Join-Path -Path $installRoot -ChildPath 'LICENSE') -Content 'license'
    Set-TestBinaryFile -Path (Join-Path -Path $installRoot -ChildPath 'ATTRIBUTIONS.md') -Content '# Attributions'
    Set-TestBinaryFile -Path (Join-Path -Path $installRoot -ChildPath 'RELEASE-MANIFEST.md') -Content '# Manifest'
    Set-TestBinaryFile -Path (Join-Path -Path $installRoot -ChildPath 'scripts\Setup-MobileEdition.ps1') -Content 'Write-Output setup'
    Set-TestBinaryFile -Path (Join-Path -Path $installRoot -ChildPath 'scripts\Start-MobileEditionSetup.ps1') -Content 'Write-Output launcher'
    $sha256 = Get-TestFileSha256 -Path (Join-Path -Path $installRoot -ChildPath 'Setup-MobileEdition.exe')
    Set-SetupBundleManifest -InstallationRoot $installRoot -Version $Tag.Substring(1) -Sha256 $sha256
...
Assert-Equal $realBootstrapManifest.version 'v0.3.0' 'Real bootstrap manifest should pin the companion version.'
Assert-Equal $realBootstrapManifest.assetUrl 'https://github.com/saleemk/feedBack-mobile-edition/releases/download/v0.3.0/Setup-MobileEdition.exe' 'Real bootstrap manifest should pin the immutable future asset URL.'
...
$managedCurrentFixture = New-RouterFixture
try {
    $companionDataRoot = Get-TestCompanionDataRoot -RepositoryRoot $managedCurrentFixture.root
    $managedRoot = New-ManagedInstallationFixture -CompanionDataRoot $companionDataRoot -Tag 'v1.2.3'
    Set-CurrentInstallationRecord -CompanionDataRoot $companionDataRoot -Tag 'v1.2.3'
    Add-TestFile -Path $managedCurrentFixture.candidates[0]
    Set-BootstrapManifestRaw -RepositoryRoot $managedCurrentFixture.root -Content '{ invalid'

    $script:managedCurrentCall = $null
    $script:managedCurrentDownloadCalls = 0
    $exitCode = Invoke-MobileEditionSetupLauncher `
        -RepositoryRoot $managedCurrentFixture.root `
        -Arguments @() `
        -ProcessLauncher {
            param([string]$FilePath, [string[]]$Arguments, [string]$WorkingDirectory)
            $script:managedCurrentCall = [pscustomobject]@{
                filePath = $FilePath
                arguments = @($Arguments)
                workingDirectory = $WorkingDirectory
            }
        } `
        -TerminalLauncher { throw 'Terminal setup should not run when a valid current managed companion exists.' } `
        -BootstrapDownloader {
            $script:managedCurrentDownloadCalls += 1
            throw 'Bootstrap downloader should not run when a valid current managed companion exists.'
        } `
        -CompanionDataRoot $companionDataRoot

    Assert-Equal $exitCode 0 'Valid current managed companion should launch successfully.'
    Assert-Equal $script:managedCurrentDownloadCalls 0 'Valid current managed companion should not download bootstrap assets.'
    Assert-Equal $script:managedCurrentCall.filePath (Join-Path -Path $managedRoot -ChildPath 'Setup-MobileEdition.exe') 'Current managed companion should take precedence over bundled candidates.'
    Assert-Equal $script:managedCurrentCall.arguments[0] '--checkout' 'Current managed launch should pass checkout flag.'
    Assert-Equal $script:managedCurrentCall.arguments[1] $managedRoot 'Current managed launch should pass the managed installation root.'
    Assert-Equal $script:managedCurrentCall.workingDirectory $managedRoot 'Current managed launch should use the managed installation root as working directory.'
} finally {
    Remove-LauncherFixture -Root $managedCurrentFixture.root
}

$unicodeManagedCurrentFixture = New-RouterFixture -UnicodePath
try {
    $companionDataRoot = Get-TestCompanionDataRoot -RepositoryRoot $unicodeManagedCurrentFixture.root
    $managedRoot = New-ManagedInstallationFixture -CompanionDataRoot $companionDataRoot -Tag 'v1.2.3'
    Set-CurrentInstallationRecord -CompanionDataRoot $companionDataRoot -Tag 'v1.2.3'

    $script:unicodeManagedCurrentCall = $null
    $exitCode = Invoke-MobileEditionSetupLauncher `
        -RepositoryRoot $unicodeManagedCurrentFixture.root `
        -Arguments @() `
        -ProcessLauncher {
            param([string]$FilePath, [string[]]$Arguments, [string]$WorkingDirectory)
            $script:unicodeManagedCurrentCall = [pscustomobject]@{
                filePath = $FilePath
                arguments = @($Arguments)
                workingDirectory = $WorkingDirectory
            }
        } `
        -TerminalLauncher { throw 'Terminal setup should not run when a valid Unicode current managed companion exists.' } `
        -BootstrapDownloader { throw 'Bootstrap downloader should not run when a valid Unicode current managed companion exists.' } `
        -CompanionDataRoot $companionDataRoot

    Assert-Equal $exitCode 0 'Unicode current managed companion should launch successfully.'
    Assert-True $managedRoot.Contains([string][char]0x00F1) 'Unicode managed fixture should exercise a Unicode path.'
    Assert-Equal $script:unicodeManagedCurrentCall.filePath (Join-Path -Path $managedRoot -ChildPath 'Setup-MobileEdition.exe') 'Unicode current managed companion should launch by derived path.'
    Assert-Equal $script:unicodeManagedCurrentCall.arguments[1] $managedRoot 'Unicode current managed launch should pass the managed installation root.'
    Assert-Equal $script:unicodeManagedCurrentCall.workingDirectory $managedRoot 'Unicode current managed launch should use the managed installation root as working directory.'
} finally {
    Remove-LauncherFixture -Root $unicodeManagedCurrentFixture.root
}

$badCurrentRecordFixture = New-RouterFixture
try {
    $companionDataRoot = Get-TestCompanionDataRoot -RepositoryRoot $badCurrentRecordFixture.root
    New-ManagedInstallationFixture -CompanionDataRoot $companionDataRoot -Tag 'v1.2.3' | Out-Null
    Set-CurrentInstallationRecordRaw -CompanionDataRoot $companionDataRoot -Content '{ invalid'
    Add-TestFile -Path $badCurrentRecordFixture.candidates[0]

    $script:badCurrentCall = $null
    $capture = Invoke-WithCapturedConsoleError -Script {
        Invoke-MobileEditionSetupLauncher `
            -RepositoryRoot $badCurrentRecordFixture.root `
            -Arguments @() `
            -ProcessLauncher {
                param([string]$FilePath, [string[]]$Arguments, [string]$WorkingDirectory)
                $script:badCurrentCall = [pscustomobject]@{
                    filePath = $FilePath
                    arguments = @($Arguments)
                    workingDirectory = $WorkingDirectory
                }
            } `
            -TerminalLauncher { throw 'Terminal setup should not run when bundled fallback exists.' } `
            -BootstrapDownloader { throw 'Bootstrap downloader should not run when bundled fallback exists.' } `
            -CompanionDataRoot $companionDataRoot
    }

    Assert-Equal $capture.result[-1] 0 'Invalid current record should fall back successfully.'
    Assert-Equal $script:badCurrentCall.filePath $badCurrentRecordFixture.candidates[0] 'Invalid current record should fall back to bundled companion.'
    Assert-True $capture.error.Contains('Current managed Setup Companion unavailable.') 'Invalid current record should print a bounded current-record diagnostic.'
    Assert-True (-not $capture.error.Contains($companionDataRoot)) 'Invalid current record diagnostic should not expose local data paths.'
} finally {
    Remove-LauncherFixture -Root $badCurrentRecordFixture.root
}

$badCurrentHashFixture = New-RouterFixture
try {
    $companionDataRoot = Get-TestCompanionDataRoot -RepositoryRoot $badCurrentHashFixture.root
    $managedRoot = New-ManagedInstallationFixture -CompanionDataRoot $companionDataRoot -Tag 'v1.2.3' -CompanionContent 'original managed bytes'
    Set-CurrentInstallationRecord -CompanionDataRoot $companionDataRoot -Tag 'v1.2.3'
    Set-TestBinaryFile -Path (Join-Path -Path $managedRoot -ChildPath 'Setup-MobileEdition.exe') -Content 'tampered managed bytes'
    Add-TestFile -Path $badCurrentHashFixture.candidates[0]

    $script:badHashCall = $null
    $capture = Invoke-WithCapturedConsoleError -Script {
        Invoke-MobileEditionSetupLauncher `
            -RepositoryRoot $badCurrentHashFixture.root `
            -Arguments @() `
            -ProcessLauncher {
                param([string]$FilePath)
                $script:badHashCall = [pscustomobject]@{ filePath = $FilePath }
            } `
            -TerminalLauncher { throw 'Terminal setup should not run when bundled fallback exists.' } `
            -BootstrapDownloader { throw 'Bootstrap downloader should not run when bundled fallback exists.' } `
            -CompanionDataRoot $companionDataRoot
    ...
    Assert-Equal $capture.result[-1] 0 'Invalid current target should fall back successfully.'
    Assert-Equal $script:badHashCall.filePath $badCurrentHashFixture.candidates[0] 'Invalid current target should fall back to bundled companion.'
    Assert-True $capture.error.Contains('Current managed Setup Companion unavailable.') 'Invalid current target should print a bounded diagnostic.'
} finally {
    Remove-LauncherFixture -Root $badCurrentHashFixture.root
}

$reparseCurrentTargetFixture = New-RouterFixture
try {
    $companionDataRoot = Get-TestCompanionDataRoot -RepositoryRoot $reparseCurrentTargetFixture.root
    $externalDataRoot = Join-Path -Path $reparseCurrentTargetFixture.root -ChildPath 'external companion data'
    $realManagedRoot = New-ManagedInstallationFixture -CompanionDataRoot $externalDataRoot -Tag 'v1.2.3'
    $managedInstallationsRoot = Join-Path -Path $companionDataRoot -ChildPath 'installations'
    New-Item -ItemType Directory -Path $managedInstallationsRoot -Force | Out-Null
    New-Item -ItemType Junction -Path (Join-Path -Path $managedInstallationsRoot -ChildPath 'v1.2.3') -Target $realManagedRoot | Out-Null
    Set-CurrentInstallationRecord -CompanionDataRoot $companionDataRoot -Tag 'v1.2.3'
    Add-TestFile -Path $reparseCurrentTargetFixture.candidates[0]

    $script:reparseCurrentCall = $null
    $capture = Invoke-WithCapturedConsoleError -Script {
        Invoke-MobileEditionSetupLauncher `
            -RepositoryRoot $reparseCurrentTargetFixture.root `
            -Arguments @() `
            -ProcessLauncher {
                param([string]$FilePath)
                $script:reparseCurrentCall = [pscustomobject]@{ filePath = $FilePath }
            } `
            -TerminalLauncher { throw 'Terminal setup should not run when bundled fallback exists.' } `
            -BootstrapDownloader { throw 'Bootstrap downloader should not run when bundled fallback exists.' } `
            -CompanionDataRoot $companionDataRoot
    ...
    Assert-Equal $capture.result[-1] 0 'Reparse-point current target should fall back successfully.'
    Assert-Equal $script:reparseCurrentCall.filePath $reparseCurrentTargetFixture.candidates[0] 'Reparse-point current target should fall back to bundled companion.'
    Assert-True $capture.error.Contains('Current managed Setup Companion unavailable.') 'Reparse-point current target should print a bounded diagnostic.'
    Assert-True (-not $capture.error.Contains($companionDataRoot)) 'Reparse-point current target diagnostic should not expose local data paths.'
} finally {
    Remove-LauncherFixture -Root $reparseCurrentTargetFixture.root
}

$forbiddenCurrentArtifactFixture = New-RouterFixture
try {
    $companionDataRoot = Get-TestCompanionDataRoot -RepositoryRoot $forbiddenCurrentArtifactFixture.root
    $managedRoot = New-ManagedInstallationFixture -CompanionDataRoot $companionDataRoot -Tag 'v1.2.3'
    Set-CurrentInstallationRecord -CompanionDataRoot $companionDataRoot -Tag 'v1.2.3'
    Set-TestBinaryFile -Path (Join-Path -Path $managedRoot -ChildPath 'node_modules\pkg\index.js') -Content 'local dependency cache'
    Add-TestFile -Path $forbiddenCurrentArtifactFixture.candidates[0]

    $script:forbiddenCurrentCall = $null
    $capture = Invoke-WithCapturedConsoleError -Script {
        Invoke-MobileEditionSetupLauncher `
            -RepositoryRoot $forbiddenCurrentArtifactFixture.root `
            -Arguments @() `
            -ProcessLauncher {
                param([string]$FilePath)
                $script:forbiddenCurrentCall = [pscustomobject]@{ filePath = $FilePath }
            } `
            -TerminalLauncher { throw 'Terminal setup should not run when bundled fallback exists.' } `
            -BootstrapDownloader { throw 'Bootstrap downloader should not run when bundled fallback exists.' } `
            -CompanionDataRoot $companionDataRoot
    ...
    Assert-Equal $capture.result[-1] 0 'Forbidden current target artifact should fall back successfully.'
    Assert-Equal $script:forbiddenCurrentCall.filePath $forbiddenCurrentArtifactFixture.candidates[0] 'Forbidden current target artifact should fall back to bundled companion.'
    Assert-True $capture.error.Contains('Current managed Setup Companion unavailable.') 'Forbidden current target artifact should print a bounded diagnostic.'
} finally {
    Remove-LauncherFixture -Root $forbiddenCurrentArtifactFixture.root
}

$currentBypassFixture = New-RouterFixture
try {
    $companionDataRoot = Get-TestCompanionDataRoot -RepositoryRoot $currentBypassFixture.root
    New-ManagedInstallationFixture -CompanionDataRoot $companionDataRoot -Tag 'v1.2.3' | Out-Null
    Set-CurrentInstallationRecord -CompanionDataRoot $companionDataRoot -Tag 'v1.2.3'
    Add-TestFile -Path $currentBypassFixture.candidates[0]

    $script:currentBypassTerminalCall = $null
    $arguments = @('-WhatIf')
    $exitCode = Invoke-MobileEditionSetupLauncher `
        -RepositoryRoot $currentBypassFixture.root `
        -Arguments $arguments `
        -ProcessLauncher { throw 'Current or bundled visual companion should not run when arguments are supplied.' } `
        -TerminalLauncher {
            param([string]$SetupScript, [string[]]$Arguments)
            $script:currentBypassTerminalCall = [pscustomobject]@{
                setupScript = $SetupScript
                arguments = @($Arguments)
            }
            return 9
        } `
        -BootstrapDownloader { throw 'Bootstrap downloader should not run when arguments are supplied.' } `
        -CompanionDataRoot $companionDataRoot

    Assert-Equal $exitCode 9 'Explicit arguments should bypass current managed activation and preserve terminal exit code.'
    Assert-Equal $script:currentBypassTerminalCall.setupScript $currentBypassFixture.setupScript 'Explicit arguments should still target the checkout setup script.'
    Assert-Equal ($script:currentBypassTerminalCall.arguments -join '|') ($arguments -join '|') 'Explicit arguments should be forwarded exactly.'
} finally {
    Remove-LauncherFixture -Root $currentBypassFixture.root
}

$strictCurrentRecordCases = @(
    [pscustomobject]@{ name = 'malformed JSON'; content = '{ invalid' },
    [pscustomobject]@{ name = 'duplicate property'; content = '{ "schema": "feedback-mobile-edition.current-installation.v1", "schema": "feedback-mobile-edition.current-installation.v1", "currentTag": "v1.2.3" }' },
    [pscustomobject]@{ name = 'additional field'; content = '{ "schema": "feedback-mobile-edition.current-installation.v1", "currentTag": "v1.2.3", "path": "C:\\secret" }' },
    [pscustomobject]@{ name = 'missing field'; content = '{ "schema": "feedback-mobile-edition.current-installation.v1" }' },
    [pscustomobject]@{ name = 'wrong type'; content = '{ "schema": "feedback-mobile-edition.current-installation.v1", "currentTag": 123 }' },
    [pscustomobject]@{ name = 'wrong schema'; content = '{ "schema": "feedback-mobile-edition.current-installation.v2", "currentTag": "v1.2.3" }' },
    [pscustomobject]@{ name = 'bad tag'; content = '{ "schema": "feedback-mobile-edition.current-installation.v1", "currentTag": "1.2.3" }' }
)

foreach ($case in $strictCurrentRecordCases) {
    $currentRecordFixture = New-RouterFixture
    try {
        $companionDataRoot = Get-TestCompanionDataRoot -RepositoryRoot $currentRecordFixture.root
        New-ManagedInstallationFixture -CompanionDataRoot $companionDataRoot -Tag 'v1.2.3' | Out-Null
        Set-CurrentInstallationRecordRaw -CompanionDataRoot $companionDataRoot -Content $case.content
        $script:currentRecordTerminalCalls = 0
        $capture = Invoke-WithCapturedConsoleError -Script {
            Invoke-MobileEditionSetupLauncher `
                -RepositoryRoot $currentRecordFixture.root `
                -Arguments @() `
                -ProcessLauncher { throw "Visual companion should not run for invalid current record case $($case.name)." } `
                -TerminalLauncher {
                    $script:currentRecordTerminalCalls += 1
                    return 0
                } `
                -BootstrapDownloader { throw "Bootstrap downloader should not run for invalid current record case $($case.name)." } `
                -CompanionDataRoot $companionDataRoot
        }

        Assert-Equal $capture.result[-1] 0 "Invalid current record case '$($case.name)' should fall back successfully."
        Assert-Equal $script:currentRecordTerminalCalls 1 "Invalid current record case '$($case.name)' should reach terminal fallback."
        Assert-True $capture.error.Contains('Current managed Setup Companion unavailable.') "Invalid current record case '$($case.name)' should print a bounded diagnostic."
        Assert-True (-not $capture.error.Contains($companionDataRoot)) "Invalid current record case '$($case.name)' should not expose local data paths."
    } finally {
        Remove-LauncherFixture -Root $currentRecordFixture.root
    }
}
